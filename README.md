...
`NpyLabelNet` — это легковесная сверточная нейронная сеть (CNN), которая классифицирует RGB-изображения размером 512x512 пикселей на 1000 классов и сохраняет предсказанные идентификаторы классов в файлы `.npy` (тип `np.int64`). Проект создан для генерации меток для нейронной сети [TintoraAI](https://github.com/T1desLuck/tintora-ai), предназначенной для раскрашивания черно-белых или выцветших фотографий, но может использоваться и для других задач классификации изображений.

Репозиторий включает скрипты для обучения модели (`train.py`), инференса с созданием `.npy` меток (`auto_label.py`) и набор тестов (`tests/`). Автоматизация проверки кода реализована через GitHub Actions.
...
- Зависимости из `requirements.txt`:
  ...
  - `numpy>=1.24.0`
  - `pillow>=9.0.0`
- Набор данных с изображениями 512x512 (подробности в `DATASET_GUIDE.md`).
...
`NpyLabelNet` разработан для поддержки [TintoraAI](https://github.com/T1desLuck/tintora-ai), которая раскрашивает черно-белые или выцветшие фотографии. TintoraAI требует:
- Папку `bw/` с черно-белыми изображениями.
- Папку `color/` с соответствующими цветными версиями.
- Папку `labels/` с `.npy` файлами меток, созданными `NpyLabelNet`.

Имена файлов в `bw/`, `color/` и `labels/` должны совпадать (например, `img_0001.png` и `img_0001.npy`). Подробности подготовки датасета — в `DATASET_GUIDE.md`.
